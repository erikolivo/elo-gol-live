...
def _api_football_request(endpoint, params=None, timeout=20, intentos=3):
    ...
    # API-Football a veces responde 200 OK con "response": [] y el
    # problema real (cupo agotado, plan insuficiente, parametro invalido,
    # etc.) descrito solo en "errors" -- sin este chequeo, este caso se
    # confunde con "no hay partidos/resultados" y falla en silencio.
    if errores:
        print(f"[AVISO] API-Football respondio 200 pero con errores para {endpoint}: {errores}")
        if any("limit" in str(v).lower() or "rate" in str(v).lower() or "quota" in str(v).lower()
               for v in (errores.values() if isinstance(errores, dict) else errores)):
            marcar_agotado()
